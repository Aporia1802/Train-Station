/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/GUIForms/JPanel.java to edit this template
 */
package gui.quanLy;

import bus.QuanLyKhuyenMai_BUS;
import entity.KhuyenMai;
import java.awt.Color;
import java.time.LocalDate;
import java.time.ZoneId;
import java.time.format.DateTimeFormatter;
import java.util.ArrayList;
import javax.swing.JOptionPane;
import javax.swing.table.DefaultTableModel;
import static utils.FormatUtil.formatCurrency;
import static utils.FormatUtil.formatDate;

/**
 *
 * @author Laptopone
 */
public class QuanLyKhuyenMai_GUI extends javax.swing.JPanel {

    private QuanLyKhuyenMai_BUS bus;
    private DefaultTableModel tblModel_thongTinKhuyenMai;
    private KhuyenMai khuyenMai;

    /**
     * Creates new form TraCuuHanhKhach_GUI
     */
    public QuanLyKhuyenMai_GUI() {
        initComponents();
        init();
    }

    private void init() {
        bus = new QuanLyKhuyenMai_BUS();

        //setModel
        tblModel_thongTinKhuyenMai = new DefaultTableModel(new String[]{"Mã KM", "Tên KM", "Mức giảm", "Ngày bắt đầu", "Ngày kết thúc", "Tổng tiền tối thiểu", "Tiền khuyến mãi tối đa", "Trạng thái"}, 0);
        tbl_thongTinKhuyenMai.setModel(tblModel_thongTinKhuyenMai);
        getTableData(bus.getAllKhuyenMai());
    }

    private void getTableData(ArrayList<KhuyenMai> dsKhuyenMai) {
        tblModel_thongTinKhuyenMai.setRowCount(0);
        for (KhuyenMai khuyenMai : dsKhuyenMai) {
            String[] newRow = {khuyenMai.getMaKhuyenMai(), khuyenMai.getTenKhuyenMai(),
                String.valueOf(khuyenMai.getHeSoKhuyenMai() * 100) + "%", String.valueOf(khuyenMai.getNgayBatDau()),
                String.valueOf(khuyenMai.getNgayKetThuc()), String.valueOf(khuyenMai.getTongTienToiThieu()),
                String.valueOf(khuyenMai.getTienKhuyenMaiToiDa()), khuyenMai.getTrangThai() ? "Còn hiệu lực" : "Hết hạn"};
            tblModel_thongTinKhuyenMai.addRow(newRow);
        }
    }

    private void getThongTinKhuyenMai() {
        int row = tbl_thongTinKhuyenMai.getSelectedRow(); // lấy dòng được chọn
        if (row != -1) {
            // Lấy dữ liệu từng cột trong dòng đó
            String maKhuyenMai = tbl_thongTinKhuyenMai.getValueAt(row, 0).toString();
            String tenKhuyenMai = tbl_thongTinKhuyenMai.getValueAt(row, 1).toString();
            String mucGiam = tbl_thongTinKhuyenMai.getValueAt(row, 2).toString();
            String ngayBatDau = tbl_thongTinKhuyenMai.getValueAt(row, 3).toString();
            String ngayKetThuc = tbl_thongTinKhuyenMai.getValueAt(row, 4).toString();
            String tienToiThieu = tbl_thongTinKhuyenMai.getValueAt(row, 5).toString();
            String tienKMToiDa = tbl_thongTinKhuyenMai.getValueAt(row, 6).toString();
            String trangThai = tbl_thongTinKhuyenMai.getValueAt(row, 7).toString();

            // Đổ lên textfield
            txt_maKhuyenMai.setText(maKhuyenMai);
            txt_tenKhuyenMai.setText(tenKhuyenMai);
            txt_mucGiam.setText(mucGiam);

            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate localDate = LocalDate.parse(ngayBatDau, formatter);
            java.util.Date ngayBD = java.sql.Date.valueOf(localDate);
            txt_ngayBatDau.setDate(ngayBD); // Gán vào JDateChooser

            localDate = LocalDate.parse(ngayKetThuc, formatter);
            java.util.Date ngayKT = java.sql.Date.valueOf(localDate);
            txt_ngayKetThuc.setDate(ngayKT); // Gán vào JDateChooser

            txt_tongTienToiThieu.setText(tienToiThieu);
            txt_tienKhuyenMaiToiDa.setText(tienKMToiDa);
            cbo_trangThai.setSelectedItem(trangThai);
        }
    }

    private void handleActionXoaTrang() {
        tbl_thongTinKhuyenMai.clearSelection();
        txt_maKhuyenMai.setText("");
        txt_tenKhuyenMai.setText("");
        txt_mucGiam.setText("");
        txt_tongTienToiThieu.setText("");
        txt_tienKhuyenMaiToiDa.setText("");

        // Xóa ngày trong JDateChooser
        txt_ngayBatDau.setDate(new java.util.Date());
        txt_ngayKetThuc.setDate(new java.util.Date());

        cbo_trangThai.setSelectedIndex(0);
    }

    private void handleActionLamMoi() {
        tbl_thongTinKhuyenMai.clearSelection();
        getTableData(bus.getAllKhuyenMai());
        txt_timKiem.setText("Nhập mã hoặc tên khuyến mãi cần tìm...");
        txt_timKiem.setForeground(Color.GRAY);
        cbo_trangThai.setSelectedIndex(0);
    }

    private void handleActionTimKiem() {
        String keyword = txt_timKiem.getText().trim();
        getTableData(bus.getGaTauByKeyword(keyword));
    }

    private void handleActionLoc() {
        String trangThai = cbo_trangThaiKM.getSelectedItem().toString().trim();
        getTableData(bus.filterByTrangThai(trangThai));
    }

    private void handleActionThem() {
        try {
            String maKM = bus.generateID();
            String tenKM = txt_tenKhuyenMai.getText().trim();
            String mucGiamStr = txt_mucGiam.getText().trim();
            java.util.Date utilDateBD = txt_ngayBatDau.getDate();
            java.util.Date utilDateKT = txt_ngayKetThuc.getDate();
            String tongTienTTStr = txt_tongTienToiThieu.getText().trim();
            String tienKMToiDaStr = txt_tienKhuyenMaiToiDa.getText().trim();
            boolean trangThai = cbo_trangThai.getSelectedItem().toString().equals("Còn hiệu lực");

            // Kiểm tra ngày không được null
            if (utilDateBD == null || utilDateKT == null) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn đầy đủ ngày bắt đầu và ngày kết thúc!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                return;
            }

            LocalDate ngayBatDau = utilDateBD.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate ngayKetThuc = utilDateKT.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

            // Kiểm tra các trường bắt buộc
            if (tenKM.isEmpty()) {
                JOptionPane.showMessageDialog(this, KhuyenMai.TENKHUYENMAI_EMPTY, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tenKhuyenMai.requestFocus();
                return;
            }

            double heSo;
            try {
                heSo = Double.parseDouble(mucGiamStr) / 100.0; // Người dùng nhập %, chuyển thành hệ số
                if (heSo <= 0 || heSo > 1) {
                    throw new Exception();
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, KhuyenMai.HESO_INVALID, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_mucGiam.requestFocus();
                return;
            }

            double tongTienTT;
            try {
                tongTienTT = Double.parseDouble(tongTienTTStr);
                if (tongTienTT <= 0) {
                    throw new Exception();
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, KhuyenMai.TONGTIEN_INVALID, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tongTienToiThieu.requestFocus();
                return;
            }

            double tienKMToiDa;
            try {
                tienKMToiDa = Double.parseDouble(tienKMToiDaStr);
                if (tienKMToiDa < 0) {
                    JOptionPane.showMessageDialog(this, "Tiền khuyến mãi tối đa không được âm!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                    txt_tienKhuyenMaiToiDa.requestFocus();
                    return;
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Tiền khuyến mãi tối đa phải là số hợp lệ!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tienKhuyenMaiToiDa.requestFocus();
                return;
            }

            // Tạo đối tượng và dùng constructor có kiểm tra
            KhuyenMai km = new KhuyenMai(maKM, tenKM, heSo, ngayBatDau, ngayKetThuc, tongTienTT, tienKMToiDa, trangThai);

            if (bus.themKhuyenMai(km)) {
                JOptionPane.showMessageDialog(this, "Thêm khuyến mãi thành công!", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                getTableData(bus.getAllKhuyenMai());
                handleActionXoaTrang();
            } else {
                JOptionPane.showMessageDialog(this, "Không thể thêm khuyến mãi! Vui lòng thử lại.", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }

        } catch (Exception e) {
            // Lỗi từ ràng buộc trong entity
            JOptionPane.showMessageDialog(this, e.getMessage(), "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void handleActionCapNhat() {
        try {
            int row = tbl_thongTinKhuyenMai.getSelectedRow();
            if (row == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn một khuyến mãi để cập nhật!", "Thông báo", JOptionPane.WARNING_MESSAGE);
                return;
            }

            String maKM = txt_maKhuyenMai.getText().trim();
            String tenKM = txt_tenKhuyenMai.getText().trim();
            String mucGiamStr = txt_mucGiam.getText().trim();
            java.util.Date utilDateBD = txt_ngayBatDau.getDate();
            java.util.Date utilDateKT = txt_ngayKetThuc.getDate();
            String tongTienTTStr = txt_tongTienToiThieu.getText().trim();
            String tienKMToiDaStr = txt_tienKhuyenMaiToiDa.getText().trim();
            boolean trangThai = cbo_trangThai.getSelectedItem().toString().equals("Còn hiệu lực");

            // Kiểm tra ngày
            if (utilDateBD == null || utilDateKT == null) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn đầy đủ ngày bắt đầu và ngày kết thúc!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                return;
            }

            LocalDate ngayBatDau = utilDateBD.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate ngayKetThuc = utilDateKT.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

            if (tenKM.isEmpty()) {
                JOptionPane.showMessageDialog(this, KhuyenMai.TENKHUYENMAI_EMPTY, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tenKhuyenMai.requestFocus();
                return;
            }

            double heSo;
            try {
                heSo = Double.parseDouble(mucGiamStr) / 100.0;
                if (heSo <= 0 || heSo > 1) {
                    throw new Exception();
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, KhuyenMai.HESO_INVALID, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_mucGiam.requestFocus();
                return;
            }

            double tongTienTT;
            try {
                tongTienTT = Double.parseDouble(tongTienTTStr);
                if (tongTienTT <= 0) {
                    throw new Exception();
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, KhuyenMai.TONGTIEN_INVALID, "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tongTienToiThieu.requestFocus();
                return;
            }

            double tienKMToiDa;
            try {
                tienKMToiDa = Double.parseDouble(tienKMToiDaStr);
                if (tienKMToiDa < 0) {
                    JOptionPane.showMessageDialog(this, "Tiền khuyến mãi tối đa không được âm!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                    txt_tienKhuyenMaiToiDa.requestFocus();
                    return;
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Tiền khuyến mãi tối đa phải là số hợp lệ!", "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
                txt_tienKhuyenMaiToiDa.requestFocus();
                return;
            }

            // Tạo đối tượng với kiểm tra ràng buộc
            KhuyenMai km = new KhuyenMai(maKM, tenKM, heSo, ngayBatDau, ngayKetThuc, tongTienTT, tienKMToiDa, trangThai);

            if (bus.capNhatKhuyenMai(km)) {
                JOptionPane.showMessageDialog(this, "Cập nhật khuyến mãi thành công!", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                getTableData(bus.getAllKhuyenMai());
                tbl_thongTinKhuyenMai.setRowSelectionInterval(row, row);
            } else {
                JOptionPane.showMessageDialog(this, "Không thể cập nhật khuyến mãi!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }

        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage(), "Lỗi nhập liệu", JOptionPane.ERROR_MESSAGE);
        }
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        buttonGroup1 = new javax.swing.ButtonGroup();
        pnlNorth = new javax.swing.JPanel();
        pnl_timKiem = new javax.swing.JPanel();
        txt_timKiem = new javax.swing.JTextField();
        pnl_btnTimKiem = new javax.swing.JPanel();
        btn_timKiem = new javax.swing.JButton();
        pnl_cta = new javax.swing.JPanel();
        cbo_trangThaiKM = new javax.swing.JComboBox<>();
        btn_Loc = new javax.swing.JButton();
        btn_Reset = new javax.swing.JButton();
        pnl_center = new javax.swing.JPanel();
        spl_container = new javax.swing.JSplitPane();
        jScrollPane1 = new javax.swing.JScrollPane();
        tbl_thongTinKhuyenMai = new javax.swing.JTable();
        pnl_container = new javax.swing.JPanel();
        pnl_button = new javax.swing.JPanel();
        btn_taoKM = new javax.swing.JButton();
        jPanel1 = new javax.swing.JPanel();
        btn_xoaTrang = new javax.swing.JButton();
        btn_capNhat = new javax.swing.JButton();
        pnl_thongTin = new javax.swing.JPanel();
        pnl_MaNV = new javax.swing.JPanel();
        lbl_maKM = new javax.swing.JLabel();
        txt_maKhuyenMai = new javax.swing.JTextField();
        pnl_tenKM = new javax.swing.JPanel();
        lbl_tenKM = new javax.swing.JLabel();
        txt_tenKhuyenMai = new javax.swing.JTextField();
        pnl_mucGiam = new javax.swing.JPanel();
        lbl_mucGiam = new javax.swing.JLabel();
        txt_mucGiam = new javax.swing.JTextField();
        pnl_ngayBatDau = new javax.swing.JPanel();
        lbl_ngayBatDau = new javax.swing.JLabel();
        txt_ngayBatDau = new com.toedter.calendar.JDateChooser();
        pnl_ngayKetThuc = new javax.swing.JPanel();
        lbl_ngayKetThuc = new javax.swing.JLabel();
        txt_ngayKetThuc = new com.toedter.calendar.JDateChooser();
        pnl_tongTienToiThieu = new javax.swing.JPanel();
        lbl_tongTienToiThieu = new javax.swing.JLabel();
        txt_tongTienToiThieu = new javax.swing.JTextField();
        pnl_tienKhuyenMaiToiDa = new javax.swing.JPanel();
        lbl_tienKhuyenMaiToiDa = new javax.swing.JLabel();
        txt_tienKhuyenMaiToiDa = new javax.swing.JTextField();
        pnl_trangThai = new javax.swing.JPanel();
        lbl_trangThai = new javax.swing.JLabel();
        cbo_trangThai = new javax.swing.JComboBox<>();

        setLayout(new java.awt.BorderLayout());

        pnlNorth.setPreferredSize(new java.awt.Dimension(1366, 55));
        pnlNorth.setLayout(new javax.swing.BoxLayout(pnlNorth, javax.swing.BoxLayout.LINE_AXIS));

        pnl_timKiem.setLayout(new javax.swing.BoxLayout(pnl_timKiem, javax.swing.BoxLayout.LINE_AXIS));

        txt_timKiem.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        txt_timKiem.setForeground(new java.awt.Color(153, 153, 153));
        txt_timKiem.setText("Nhập mã hoặc tên khuyến mãi cần tìm...");
        txt_timKiem.setPreferredSize(new java.awt.Dimension(700, 30));
        txt_timKiem.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusGained(java.awt.event.FocusEvent evt) {
                txt_timKiemFocusGained(evt);
            }
            public void focusLost(java.awt.event.FocusEvent evt) {
                txt_timKiemFocusLost(evt);
            }
        });
        pnl_timKiem.add(txt_timKiem);

        pnl_btnTimKiem.setLayout(new java.awt.BorderLayout());

        btn_timKiem.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        btn_timKiem.setText("Tìm kiếm");
        btn_timKiem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_timKiemActionPerformed(evt);
            }
        });
        pnl_btnTimKiem.add(btn_timKiem, java.awt.BorderLayout.CENTER);

        pnl_timKiem.add(pnl_btnTimKiem);

        pnlNorth.add(pnl_timKiem);

        pnl_cta.setPreferredSize(new java.awt.Dimension(500, 50));
        pnl_cta.setLayout(new java.awt.GridLayout(1, 0));

        cbo_trangThaiKM.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        cbo_trangThaiKM.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Trạng Thái", "Còn hiệu lực", "Hết hạn" }));
        pnl_cta.add(cbo_trangThaiKM);

        btn_Loc.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        btn_Loc.setText("Lọc");
        btn_Loc.setMaximumSize(new java.awt.Dimension(100, 50));
        btn_Loc.setPreferredSize(new java.awt.Dimension(100, 50));
        btn_Loc.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_LocActionPerformed(evt);
            }
        });
        pnl_cta.add(btn_Loc);

        btn_Reset.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        btn_Reset.setText("Reset");
        btn_Reset.setMaximumSize(new java.awt.Dimension(100, 50));
        btn_Reset.setPreferredSize(new java.awt.Dimension(100, 50));
        btn_Reset.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                btn_ResetMouseClicked(evt);
            }
        });
        pnl_cta.add(btn_Reset);

        pnlNorth.add(pnl_cta);

        add(pnlNorth, java.awt.BorderLayout.PAGE_START);

        pnl_center.setLayout(new javax.swing.BoxLayout(pnl_center, javax.swing.BoxLayout.LINE_AXIS));

        spl_container.setResizeWeight(0.8);
        spl_container.setMinimumSize(new java.awt.Dimension(805, 416));
        spl_container.setPreferredSize(new java.awt.Dimension(1055, 718));

        jScrollPane1.setBackground(new java.awt.Color(255, 255, 255));
        jScrollPane1.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(204, 204, 204)));

        tbl_thongTinKhuyenMai.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null}
            },
            new String [] {
                "Title 1", "Title 2", "Title 3", "Title 4", "Title 5", "Title 6", "Title 7"
            }
        ));
        tbl_thongTinKhuyenMai.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tbl_thongTinKhuyenMaiMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(tbl_thongTinKhuyenMai);

        spl_container.setLeftComponent(jScrollPane1);

        pnl_container.setPreferredSize(new java.awt.Dimension(400, 668));
        pnl_container.setLayout(new java.awt.BorderLayout());

        pnl_button.setBackground(new java.awt.Color(255, 255, 255));
        pnl_button.setMaximumSize(new java.awt.Dimension(32767, 100));
        pnl_button.setMinimumSize(new java.awt.Dimension(277, 100));
        pnl_button.setPreferredSize(new java.awt.Dimension(390, 100));
        pnl_button.setLayout(new java.awt.BorderLayout());

        btn_taoKM.setBackground(new java.awt.Color(123, 17, 19));
        btn_taoKM.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        btn_taoKM.setForeground(new java.awt.Color(255, 255, 255));
        btn_taoKM.setText("Tạo khuyến mãi");
        btn_taoKM.setPreferredSize(new java.awt.Dimension(136, 50));
        btn_taoKM.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_taoKMActionPerformed(evt);
            }
        });
        pnl_button.add(btn_taoKM, java.awt.BorderLayout.PAGE_END);

        jPanel1.setLayout(new java.awt.GridLayout(1, 2, 5, 5));

        btn_xoaTrang.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        btn_xoaTrang.setText("Xóa trắng");
        btn_xoaTrang.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_xoaTrangActionPerformed(evt);
            }
        });
        jPanel1.add(btn_xoaTrang);

        btn_capNhat.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        btn_capNhat.setText("Cập nhật");
        btn_capNhat.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_capNhatActionPerformed(evt);
            }
        });
        jPanel1.add(btn_capNhat);

        pnl_button.add(jPanel1, java.awt.BorderLayout.CENTER);

        pnl_container.add(pnl_button, java.awt.BorderLayout.PAGE_END);

        pnl_thongTin.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createTitledBorder("Thông tin khuyến mãi"), javax.swing.BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        pnl_thongTin.setLayout(new javax.swing.BoxLayout(pnl_thongTin, javax.swing.BoxLayout.Y_AXIS));

        pnl_MaNV.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_MaNV.setMinimumSize(new java.awt.Dimension(118, 6));
        pnl_MaNV.setPreferredSize(new java.awt.Dimension(230, 50));
        pnl_MaNV.setLayout(new javax.swing.BoxLayout(pnl_MaNV, javax.swing.BoxLayout.X_AXIS));

        lbl_maKM.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_maKM.setText("Mã KM:");
        lbl_maKM.setMaximumSize(new java.awt.Dimension(45, 16));
        lbl_maKM.setMinimumSize(new java.awt.Dimension(45, 16));
        lbl_maKM.setPreferredSize(new java.awt.Dimension(120, 16));
        pnl_MaNV.add(lbl_maKM);

        txt_maKhuyenMai.setCursor(new java.awt.Cursor(java.awt.Cursor.TEXT_CURSOR));
        txt_maKhuyenMai.setEnabled(false);
        txt_maKhuyenMai.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        txt_maKhuyenMai.setPreferredSize(new java.awt.Dimension(64, 40));
        pnl_MaNV.add(txt_maKhuyenMai);

        pnl_thongTin.add(pnl_MaNV);

        pnl_tenKM.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_tenKM.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_tenKM.setLayout(new javax.swing.BoxLayout(pnl_tenKM, javax.swing.BoxLayout.LINE_AXIS));

        lbl_tenKM.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_tenKM.setLabelFor(lbl_tenKM);
        lbl_tenKM.setText("Tên KM:");
        lbl_tenKM.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_tenKM.add(lbl_tenKM);

        txt_tenKhuyenMai.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        txt_tenKhuyenMai.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        pnl_tenKM.add(txt_tenKhuyenMai);

        pnl_thongTin.add(pnl_tenKM);

        pnl_mucGiam.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_mucGiam.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_mucGiam.setLayout(new javax.swing.BoxLayout(pnl_mucGiam, javax.swing.BoxLayout.LINE_AXIS));

        lbl_mucGiam.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_mucGiam.setLabelFor(pnl_mucGiam);
        lbl_mucGiam.setText("Mức giảm:");
        lbl_mucGiam.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_mucGiam.add(lbl_mucGiam);

        txt_mucGiam.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        txt_mucGiam.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        pnl_mucGiam.add(txt_mucGiam);

        pnl_thongTin.add(pnl_mucGiam);

        pnl_ngayBatDau.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_ngayBatDau.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_ngayBatDau.setLayout(new javax.swing.BoxLayout(pnl_ngayBatDau, javax.swing.BoxLayout.LINE_AXIS));

        lbl_ngayBatDau.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_ngayBatDau.setLabelFor(txt_ngayBatDau);
        lbl_ngayBatDau.setText("Ngày bắt đầu:");
        lbl_ngayBatDau.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_ngayBatDau.add(lbl_ngayBatDau);

        txt_ngayBatDau.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        txt_ngayBatDau.setMinimumSize(new java.awt.Dimension(141, 0));
        pnl_ngayBatDau.add(txt_ngayBatDau);

        pnl_thongTin.add(pnl_ngayBatDau);

        pnl_ngayKetThuc.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_ngayKetThuc.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_ngayKetThuc.setLayout(new javax.swing.BoxLayout(pnl_ngayKetThuc, javax.swing.BoxLayout.LINE_AXIS));

        lbl_ngayKetThuc.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_ngayKetThuc.setLabelFor(txt_ngayKetThuc);
        lbl_ngayKetThuc.setText("Ngày kết thúc:");
        lbl_ngayKetThuc.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_ngayKetThuc.add(lbl_ngayKetThuc);

        txt_ngayKetThuc.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        txt_ngayKetThuc.setMinimumSize(new java.awt.Dimension(141, 0));
        pnl_ngayKetThuc.add(txt_ngayKetThuc);

        pnl_thongTin.add(pnl_ngayKetThuc);

        pnl_tongTienToiThieu.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_tongTienToiThieu.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_tongTienToiThieu.setLayout(new javax.swing.BoxLayout(pnl_tongTienToiThieu, javax.swing.BoxLayout.LINE_AXIS));

        lbl_tongTienToiThieu.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_tongTienToiThieu.setLabelFor(pnl_tongTienToiThieu);
        lbl_tongTienToiThieu.setText("Tổng tiền tối thiểu:");
        lbl_tongTienToiThieu.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_tongTienToiThieu.add(lbl_tongTienToiThieu);

        txt_tongTienToiThieu.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        txt_tongTienToiThieu.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        pnl_tongTienToiThieu.add(txt_tongTienToiThieu);

        pnl_thongTin.add(pnl_tongTienToiThieu);

        pnl_tienKhuyenMaiToiDa.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_tienKhuyenMaiToiDa.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_tienKhuyenMaiToiDa.setLayout(new javax.swing.BoxLayout(pnl_tienKhuyenMaiToiDa, javax.swing.BoxLayout.LINE_AXIS));

        lbl_tienKhuyenMaiToiDa.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_tienKhuyenMaiToiDa.setLabelFor(pnl_tienKhuyenMaiToiDa);
        lbl_tienKhuyenMaiToiDa.setText("Tiền KM tối đa:");
        lbl_tienKhuyenMaiToiDa.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_tienKhuyenMaiToiDa.add(lbl_tienKhuyenMaiToiDa);

        txt_tienKhuyenMaiToiDa.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        txt_tienKhuyenMaiToiDa.setMaximumSize(new java.awt.Dimension(2147483647, 40));
        pnl_tienKhuyenMaiToiDa.add(txt_tienKhuyenMaiToiDa);

        pnl_thongTin.add(pnl_tienKhuyenMaiToiDa);

        pnl_trangThai.setMaximumSize(new java.awt.Dimension(2147483647, 50));
        pnl_trangThai.setPreferredSize(new java.awt.Dimension(390, 50));
        pnl_trangThai.setLayout(new javax.swing.BoxLayout(pnl_trangThai, javax.swing.BoxLayout.LINE_AXIS));

        lbl_trangThai.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lbl_trangThai.setLabelFor(pnl_mucGiam);
        lbl_trangThai.setText("Trạng thái:");
        lbl_trangThai.setPreferredSize(new java.awt.Dimension(120, 20));
        pnl_trangThai.add(lbl_trangThai);

        cbo_trangThai.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Còn hiệu lực", "Hết hạn" }));
        cbo_trangThai.setMaximumSize(new java.awt.Dimension(32767, 40));
        pnl_trangThai.add(cbo_trangThai);

        pnl_thongTin.add(pnl_trangThai);

        pnl_container.add(pnl_thongTin, java.awt.BorderLayout.CENTER);

        spl_container.setRightComponent(pnl_container);

        pnl_center.add(spl_container);

        add(pnl_center, java.awt.BorderLayout.CENTER);
    }// </editor-fold>//GEN-END:initComponents

    private void btn_taoKMActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_taoKMActionPerformed
        // TODO add your handling code here:
        handleActionThem();
    }//GEN-LAST:event_btn_taoKMActionPerformed

    private void btn_xoaTrangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_xoaTrangActionPerformed
        // TODO add your handling code here:
        handleActionXoaTrang();
    }//GEN-LAST:event_btn_xoaTrangActionPerformed

    private void tbl_thongTinKhuyenMaiMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_thongTinKhuyenMaiMouseClicked
        // TODO add your handling code here:
        getThongTinKhuyenMai();
    }//GEN-LAST:event_tbl_thongTinKhuyenMaiMouseClicked

    private void btn_ResetMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_ResetMouseClicked
        // TODO add your handling code here:
        handleActionLamMoi();
    }//GEN-LAST:event_btn_ResetMouseClicked

    private void txt_timKiemFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txt_timKiemFocusGained
        // TODO add your handling code here:
        if (txt_timKiem.getText().equals("Nhập mã hoặc tên khuyến mãi cần tìm...")) {
            txt_timKiem.setText("");
            txt_timKiem.setForeground(Color.BLACK);
        }
    }//GEN-LAST:event_txt_timKiemFocusGained

    private void txt_timKiemFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txt_timKiemFocusLost
        // TODO add your handling code here:
        if (txt_timKiem.getText().equals("")) {
            txt_timKiem.setText("Nhập mã hoặc tên khuyến mãi cần tìm...");
            txt_timKiem.setForeground(Color.GRAY);
        }
    }//GEN-LAST:event_txt_timKiemFocusLost

    private void btn_timKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_timKiemActionPerformed
        // TODO add your handling code here:
        handleActionTimKiem();
    }//GEN-LAST:event_btn_timKiemActionPerformed

    private void btn_LocActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_LocActionPerformed
        // TODO add your handling code here:
        handleActionLoc();
    }//GEN-LAST:event_btn_LocActionPerformed

    private void btn_capNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_capNhatActionPerformed
        // TODO add your handling code here:
        handleActionCapNhat();
    }//GEN-LAST:event_btn_capNhatActionPerformed


    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton btn_Loc;
    private javax.swing.JButton btn_Reset;
    private javax.swing.JButton btn_capNhat;
    private javax.swing.JButton btn_taoKM;
    private javax.swing.JButton btn_timKiem;
    private javax.swing.JButton btn_xoaTrang;
    private javax.swing.ButtonGroup buttonGroup1;
    private javax.swing.JComboBox<String> cbo_trangThai;
    private javax.swing.JComboBox<String> cbo_trangThaiKM;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JLabel lbl_maKM;
    private javax.swing.JLabel lbl_mucGiam;
    private javax.swing.JLabel lbl_ngayBatDau;
    private javax.swing.JLabel lbl_ngayKetThuc;
    private javax.swing.JLabel lbl_tenKM;
    private javax.swing.JLabel lbl_tienKhuyenMaiToiDa;
    private javax.swing.JLabel lbl_tongTienToiThieu;
    private javax.swing.JLabel lbl_trangThai;
    private javax.swing.JPanel pnlNorth;
    private javax.swing.JPanel pnl_MaNV;
    private javax.swing.JPanel pnl_btnTimKiem;
    private javax.swing.JPanel pnl_button;
    private javax.swing.JPanel pnl_center;
    private javax.swing.JPanel pnl_container;
    private javax.swing.JPanel pnl_cta;
    private javax.swing.JPanel pnl_mucGiam;
    private javax.swing.JPanel pnl_ngayBatDau;
    private javax.swing.JPanel pnl_ngayKetThuc;
    private javax.swing.JPanel pnl_tenKM;
    private javax.swing.JPanel pnl_thongTin;
    private javax.swing.JPanel pnl_tienKhuyenMaiToiDa;
    private javax.swing.JPanel pnl_timKiem;
    private javax.swing.JPanel pnl_tongTienToiThieu;
    private javax.swing.JPanel pnl_trangThai;
    private javax.swing.JSplitPane spl_container;
    private javax.swing.JTable tbl_thongTinKhuyenMai;
    private javax.swing.JTextField txt_maKhuyenMai;
    private javax.swing.JTextField txt_mucGiam;
    private com.toedter.calendar.JDateChooser txt_ngayBatDau;
    private com.toedter.calendar.JDateChooser txt_ngayKetThuc;
    private javax.swing.JTextField txt_tenKhuyenMai;
    private javax.swing.JTextField txt_tienKhuyenMaiToiDa;
    private javax.swing.JTextField txt_timKiem;
    private javax.swing.JTextField txt_tongTienToiThieu;
    // End of variables declaration//GEN-END:variables
}
